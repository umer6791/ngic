...
process_create_bearer_response(gtpv2c_header *gtpv2c_rx)
{
	...
	session.ue_addr.u.ipv4_addr =
		ntohl(create_bearer_rsp.ded_bearer->pdn->ipv4.s_addr);
	session.ul_s1_info.sgw_teid =
		ntohl(create_bearer_rsp.ded_bearer->s1u_sgw_gtpu_teid),
	...
	session.ul_s1_info.enb_addr.u.ipv4_addr =
		ntohl(create_bearer_rsp.ded_bearer->s1u_enb_gtpu_ipv4.s_addr);
	session.dl_s1_info.enb_teid =
		ntohl(create_bearer_rsp.ded_bearer->s1u_enb_gtpu_teid);
