...
int
create_ue_context(gtpv2c_ie *imsi_ie, uint8_t ebi, ue_context **context)
{
	int ret;
	int i;
	uint8_t ebi_index;
...
{
	if (apn_length != one_apn.apn_name_length
	    || strncmp(apn_label, one_apn.apn_name_label, apn_length)) {
		fprintf(stderr,
